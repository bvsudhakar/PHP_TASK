...
if (isset($_POST['add'])) {
    ...
    $stu_sql = mysqli_query($conn, "SELECT * FROM `tbl_students` WHERE `name`='$name' AND `subject`='$subject' AND `teacher_id`='".$_SESSION['admin_id']."' ");
    $stu_count = mysqli_num_rows($stu_sql);

    if($stu_count){
        $date = date('d-m-Y H:i:s');
        $sql = mysqli_query($conn, "UPDATE `tbl_students` SET `marks`='$marks', `updated_at`='$date' WHERE `name`='$name' AND `subject`='$subject' ");
    
if($sql){
    $success = "success";
    header("location:dashboard.php?msg=success");
}else{
    $fail = "You can't update this data";
    header("location:dashboard.php?msg=error");
}
        
    }else{
        $date = date('d-m-Y H:i:s');
        $sql = mysqli_query($conn, "INSERT INTO `tbl_students`(`teacher_id`, `name`, `subject`, `marks`, `created_at`, `updated_at`) VALUES ('".$_SESSION['admin_id']."', '$name','$subject','$marks','$date','$date')");
        if ($sql) {
            $success = "success";
            header("location:dashboard.php?msg=success");
        } else {
...
if (isset($_POST['edit'])) {
    ...
    $csql = mysqli_query($conn, "SELECT * FROM `tbl_students` WHERE `id`='$id' ");
    $ucount = mysqli_num_rows($csql);
   
    if($ucount){
    $date = date('d-m-Y H:i:s');
    $usql = mysqli_query($conn, "UPDATE `tbl_students` SET `name`='$name',`subject`='$subject',`marks`='$marks',`updated_at`='$date' WHERE `id`='$id' ");
...
$ssql = mysqli_query($conn, "SELECT * FROM `tbl_students` WHERE `teacher_id`='".$_SESSION['admin_id']."' ORDER BY id DESC");
